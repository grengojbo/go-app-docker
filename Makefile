...
APP=go-app-docker
PROJECT=github.com/grengojbo/go-app-docker
REGISTRY?=docker.io/grengojbo
CA_DIR?=certs
...
#.PHONY: run
run:
	@echo "+ $@  -> PORT=${APP_PORT} go run main.go"
	@#PORT=${APP_PORT} APPKIT_LOG_HUMAN=1 go run -ldflags "-s -w -X main.Version=${RELEASE}" main.go
	@PORT=${APP_PORT} APPKIT_LOG_HUMAN=1 LOG_LEVEL=error go run -ldflags "-s -w -X main.Version=${RELEASE}" main.go
	@#PORT=${APP_PORT} LOG_LEVEL=error go run -ldflags "-s -w -X main.Version=${RELEASE}" main.go
#	@docker run --name ${CONTAINER_NAME} -p ${APP_PORT}:${APP_PORT} \
#		-e "APP_HOST=${APP_HOST}" \
#		-e "APP_PORT=${APP_PORT}" \
